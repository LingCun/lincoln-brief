...
4. **Determine SLOT** from the `SLOT=...` line at the top of this prompt (2자리 숫자, KST 발행 시각).
   - US: `01` (01:00 KST) 또는 `06` (06:00 KST)
   - KR: `11` (11:00 KST) 또는 `16` (16:00 KST)
   - If absent, refuse and exit.

**Slug pattern — FIXED for daily batch (no exceptions, no topic-based slugs):**

```
{us|kr}-{category}-{YYYYMMDD}-{SLOT}.md
```

The 4 slugs the batch writes (exactly, derived from MARKET + today's KST date + SLOT):

- US batch (MARKET=US, SLOT=01 or 06):
  - `us-daily-brief-YYYYMMDD-{SLOT}.md`
  - `us-stock-analysis-YYYYMMDD-{SLOT}.md`
  - `us-market-forecast-YYYYMMDD-{SLOT}.md`
  - `us-economy-issue-YYYYMMDD-{SLOT}.md`
- KR batch (MARKET=KR, SLOT=11 or 16):
  - `kr-daily-brief-YYYYMMDD-{SLOT}.md`
  - `kr-stock-analysis-YYYYMMDD-{SLOT}.md`
  - `kr-market-forecast-YYYYMMDD-{SLOT}.md`
  - `kr-economy-issue-YYYYMMDD-{SLOT}.md`
...
**Slot 별 톤·소재 차이** (둘 다 같은 4편 만들지만 시각이 다름):
- 1st slot (US 01:00 / KR 11:00) — 장 마감/개장 직후 1차 정리. 핵심 사실 + 짧은 분석.
- 2nd slot (US 06:00 / KR 16:00) — 그날의 정리·갱신. 1st slot 글을 같은 카테고리 최근 글로 참조해서 톤 잇되, 새 흐름·뉴스 반영.

5. **Pick subjects for each category** from market-snapshot. Subject choice affects the `title`/`description`/`tags` of each fixed-slug article, NOT the slug itself:
   - `daily-brief` (slug `{prefix}-daily-brief-{date}-{slot}`) — overall market summary, 4-sector view
   - `stock-analysis` (slug `{prefix}-stock-analysis-{date}-{slot}`) — biggest mover from `top12` (positive or negative)
   - `market-forecast` (slug `{prefix}-market-forecast-{date}-{slot}`) — next 1-4 week scenario tied to upcoming events visible in data
   - `economy-issue` (slug `{prefix}-economy-issue-{date}-{slot}`) — top macro story implied by data (FX, rates, geopolitics)

6. **Fetch news context (optional but recommended):**
   - For the stock you picked in #5 stock-analysis, use `WebFetch` on the Yahoo Finance news page (e.g., `https://finance.yahoo.com/quote/NVDA/news`) to get 3-5 headlines. Cite them in `sources:`.
   - If `WebFetch` is blocked, proceed with snapshot-only data.

7. **Write 4 markdown files** at `src/content/blog/<slug>.md`. Each:
...
8. **Create 4 SVG thumbnails** at `public/thumbnails/<slug>.svg`:
...
9. **Safety gate** — before committing, grep each new .md file:
...
11. **Commit & push:**
    ...
    - Commit message: `data: ${MARKET} daily brief auto-generated for ${DATE} slot ${SLOT}` (e.g., `data: US daily brief auto-generated for 2026-05-21 slot 06`).
