...
3. 다음 값 입력:
   ...
   | Schedule | Every **1 hour** (매시 :00 — 2026-05-26 부로 30분→1시간으로 줄임. 이유는 §4 참고. 더 짧게 가도 cron-job.org 무료 한도(100회/일) 안엔 들어옴. 워크플로의 `concurrency.group` 이 겹친 실행도 차단.) |
   | Request method | **POST** |
...
- cron-job.org 무료 플랜:
  ...
  - Lincoln Brief 현재 cron 합계 = `refresh-market` 24/일 (1시간 간격) + `*-daily-prep` 4슬롯/일 = **28/일** → 한도 ¼ 수준
  - 2026-05-24: 10분 간격(144/일)으로 돌렸다가 한도 초과로 막힌 적 있음. 그 후 30분(48/일) 으로 줄였다가, 2026-05-26 git history / Vercel 빌드 노이즈를 더 줄이려 **1시간(24/일)** 로 추가 완화. snapshot 신선도 손해 = 사용자가 보는 홈 카드 데이터가 최대 ~60분 stale 가능 — daily prep 본문은 prep 직전 외부 cron 한 번이 따로 들어가서 영향 없음 (워크플로 자체 cron 으로도 시간당 1번은 백업).
