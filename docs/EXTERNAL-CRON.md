...
3. 다음 값 입력:
   ...
   | Schedule | Every **20 minutes** (`*/20` 또는 매시 :00/:20/:40 — cron-job.org 무료 플랜의 일 실행 한도(100회/일) 를 피하려 일 72번으로 맞춤. 더 짧게 가면 한도 초과 알림이 옵니다. 워크플로의 `concurrency.group` 이 겹친 실행도 차단.) |
   | Request method | **POST** |
...
- cron-job.org 무료 플랜:
  ...
  - Lincoln Brief 현재 cron 합계 = `refresh-market` 72/일 (20분 간격) + `*-daily-prep` 4슬롯/일 = **76/일** → 한도 안쪽
  - 만약 `refresh-market` 을 10분 간격(144/일)으로 돌렸다가 한도 초과로 막힌 적 있음 (2026-05-24). 20분 간격이 무료 플랜의 안전 상한.
